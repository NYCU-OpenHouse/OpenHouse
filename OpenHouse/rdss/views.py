...
def Status(request):
    ...
    try:
        ...
        if signup_data.jobfair:
            ...
            elif mycompany.ece_member_normal and signup_data.jobfair >= 2 - mycompany.jobfair_spring_2026:
                # The code below should be updaed in fall 2026 because the limit "2 times" is for whole 2026 year.
                ece_discount_num = max(min(signup_data.jobfair, 2 - mycompany.jobfair_spring_2026), 0)
                if ece_discount_num > 0:
                    ece_discount = ece_discount_num * configs.jobfair_booth_fee
                    discount_text.append(f"貴公司為電機資源產學聯盟會員，2026 年最多可使用兩次博覽會攤位免費優惠，自動使用攤位優惠 {ece_discount_num} 次 -{ece_discount}元")
                    discount += ece_discount
            elif mycompany.ece_member or mycompany.ece_member_normal:
                ece_discount = min(signup_data.jobfair, 1) * configs.jobfair_booth_fee
                discount_text.append(f"貴公司為電機資源產學聯盟會員，自動使用1次 -{ece_discount}元")
                discount += ece_discount
...
def redeem_seminar_onsite_prize(request, idcard_no, date):
    ...
    try:
        student_obj = rdss.models.Student.objects.filter(idcard_no=idcard_no).first()
    except Exception as e:
    ...
    try:
        rdss.models.RedeemPrize.objects.get_or_create(
            student=student_obj, prize=f"現場獎品-{date}"
        )
        rdss.models.RedeemOnsitePrize.objects.filter(
            student=student_obj, date=date
        ).update(redeem=True)
        messages.success(request, f"學生 {student_obj.student_id} 兌換現場獎品成功，日期: {date}")
        collect_pts_logger.info('student %s succeeded to get seminar onsite prize, date: %s', student_obj.student_id, date)
    except Exception as e:
